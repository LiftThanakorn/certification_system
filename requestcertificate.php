...
if (isset($_SESSION['user_id'])) {
    ...
    if ($_SESSION['user_level'] === 'ผู้ใช้ทั่วไป') {
        ...
        if (isset($_POST['category_certificate']) && !empty($_POST['category_certificate'])) {
            ...
            $sql = "INSERT INTO requestcertificate (user_id, category_id, status, request_date, update_date, additional_data) VALUES (?, ?, 'รอดำเนินการ', NOW(), NOW(), ?)";
            $stmt = mysqli_prepare($conn, $sql);

            // Bind the parameters
            mysqli_stmt_bind_param($stmt, "iss", $user_id, $category_certificate, $additional_data);
